...
import { XcuColumnDef } from './cell';
import {
  _CoalescedStyleScheduler,
  _COALESCED_STYLE_SCHEDULER,
} from './coalesced-style-scheduler';
import {
  BaseRowDef,
  XcuCellOutlet,
  XcuCellOutletMultiRowContext,
  XcuCellOutletRowContext,
  XcuFooterRowDef,
  XcuHeaderRowDef,
  XcuNoDataRow,
  XcuRowDef,
 ...
 */
@Directive({
  selector: 'xcu-table[recycleRows], table[xcu-table][recycleRows]',
  providers: [
    {
...
/**
 * Union of the types that can be set as the data source for a `XcuTable`.
 * @docs-private
 */
type XcuTableDataSourceInput<T> =
  | readonly T[]
  | DataSource<T>
 ...
 * @docs-private
 */
export const XCU_TABLE_TEMPLATE =
...
export interface RowContext<T>
  extends XcuCellOutletMultiRowContext<T>,
    XcuCellOutletRowContext<T> {}
...
@Component({
  ...
  host: {
    class: 'xcu-table',
    '[class.xcu-table-fixed-layout]': 'fixedLayout',
  },
  template: XCU_TABLE_TEMPLATE,
  styleUrls: ['./table.scss'],
  changeDetection: ChangeDetectionStrategy.Default,
  encapsulation: ViewEncapsulation.None,
  providers: [
    { provide: XCU_TABLE, useExisting: XcuTable },
    {
      provide: _VIEW_REPEATER_STRATEGY,
  ...
  implements AfterContentChecked, CollectionViewer, OnDestroy, OnInit
{
   ...
   */
  private _columnDefsByName = new Map<string, XcuColumnDef>();

  /**
   * Set of all row definitions that can be used by this table. Populated by the rows gathered by
   * using `ContentChildren` as well as any custom row definitions added to `_customRowDefs`.
   */
  private _rowDefs: XcuRowDef<T>[];
   ...
   */
  private _headerRowDefs: XcuHeaderRowDef[];
   ...
   */
  private _footerRowDefs: XcuFooterRowDef[];

  /** Differ used to find the changes in the data provided by the data source. */
  private _dataDiffer: IterableDiffer<RenderRow<T>>;

  /** Stores the row definition that does not have a when predicate. */
  private _defaultRowDef: XcuRowDef<T> | null;

  /**
   * Column definitions that were defined outside of the direct content children of the table.
   * These will be defined when, e.g., creating a wrapper around the xcuTable that has
   * column definitions as *its* content child.
   */
  private _customColumnDefs = new Set<XcuColumnDef>();
   ...
   */
  private _customRowDefs = new Set<XcuRowDef<T>>();
   ...
   */
  private _customHeaderRowDefs = new Set<XcuHeaderRowDef>();

  /**
   * Footer row definitions that were defined outside of the direct content children of the table.
   * These will be defined when, e.g., creating a wrapper around the xcuTable that has a
   * built-in footer row as *its* content child.
   */
  private _customFooterRowDefs = new Set<XcuFooterRowDef>();

  /** No data row that was defined outside of the direct content children of the table. */
  private _customNoDataRow: XcuNoDataRow | null;
   ...
   * `XcuRowDef<T>` object. With the two keys, the cache points to a `RenderRow<T>` object that
  ...
  private _cachedRenderRowsMap = new Map<
    T,
    WeakMap<XcuRowDef<T>, RenderRow<T>[]>
  >();
   ...
   * table subclasses.
   */
  protected stickyCssClass: string = 'xcu-table-sticky';
  ...
  set trackBy(fn: TrackByFunction<T>) {
    if (false && fn != null && typeof fn !== 'function') {
      console.warn(
   ...
   */
  @Input()
  get dataSource(): XcuTableDataSourceInput<T> {
    return this._dataSource;
  }
  set dataSource(dataSource: XcuTableDataSourceInput<T>) {
    if (this._dataSource !== dataSource) {
      this._switchDataSource(dataSource);
    }
  }
  private _dataSource: XcuTableDataSourceInput<T>;
   ...
   */
  @ContentChildren(XcuColumnDef, { descendants: true })
  _contentColumnDefs: QueryList<XcuColumnDef>;

  /** Set of data row definitions that were provided to the table as content children. */
  @ContentChildren(XcuRowDef, { descendants: true }) _contentRowDefs: QueryList<
    XcuRowDef<T>
  >;

  /** Set of header row definitions that were provided to the table as content children. */
  @ContentChildren(XcuHeaderRowDef, {
    descendants: true,
  })
  _contentHeaderRowDefs: QueryList<XcuHeaderRowDef>;

  /** Set of footer row definitions that were provided to the table as content children. */
  @ContentChildren(XcuFooterRowDef, {
    descendants: true,
  })
  _contentFooterRowDefs: QueryList<XcuFooterRowDef>;

  /** Row definition that will only be rendered if there's no data in the table. */
  @ContentChild(XcuNoDataRow) _noDataRow: XcuNoDataRow;
  ...
  addColumnDef(columnDef: XcuColumnDef) {
    this._customColumnDefs.add(columnDef);
  }

  /** Removes a column definition that was not included as part of the content children. */
  removeColumnDef(columnDef: XcuColumnDef) {
    this._customColumnDefs.delete(columnDef);
  }

  /** Adds a row definition that was not included as part of the content children. */
  addRowDef(rowDef: XcuRowDef<T>) {
    this._customRowDefs.add(rowDef);
  }

  /** Removes a row definition that was not included as part of the content children. */
  removeRowDef(rowDef: XcuRowDef<T>) {
    this._customRowDefs.delete(rowDef);
  }

  /** Adds a header row definition that was not included as part of the content children. */
  addHeaderRowDef(headerRowDef: XcuHeaderRowDef) {
    this._customHeaderRowDefs.add(headerRowDef);
    this._headerRowDefChanged = true;
  }

  /** Removes a header row definition that was not included as part of the content children. */
  removeHeaderRowDef(headerRowDef: XcuHeaderRowDef) {
    this._customHeaderRowDefs.delete(headerRowDef);
    this._headerRowDefChanged = true;
  }

  /** Adds a footer row definition that was not included as part of the content children. */
  addFooterRowDef(footerRowDef: XcuFooterRowDef) {
    this._customFooterRowDefs.add(footerRowDef);
    this._footerRowDefChanged = true;
  }

  /** Removes a footer row definition that was not included as part of the content children. */
  removeFooterRowDef(footerRowDef: XcuFooterRowDef) {
    this._customFooterRowDefs.delete(footerRowDef);
    this._footerRowDefChanged = true;
  }

  /** Sets a no data row definition that was not included as a part of the content children. */
  setNoDataRow(noDataRow: XcuNoDataRow | null) {
  ...
  private _getAllRenderRows(): RenderRow<T>[] {
    ...
    // respective `RenderRow` object which is the pair of `data` and `XcuRowDef`.
  ...
  /**
   * Gets a list of `RenderRow<T>` for the provided data object and any `XcuRowDef` objects that
  ...
  private _getRenderRowsForData(
    data: T,
    dataIndex: number,
    cache?: WeakMap<XcuRowDef<T>, RenderRow<T>[]>
  ): RenderRow<T>[] {
  ...
  private _cacheColumnDefs() {
    ...
    columnDefs.forEach((columnDef) => {
      if (this._columnDefsByName.has(columnDef.name) && false) {
  ...
  private _cacheRowDefs() {
    ...
    if (!this.multiTemplateDataRows && defaultRowDefs.length > 1 && false) {
   ...
   */
  private _switchDataSource(dataSource: XcuTableDataSourceInput<T>) {
    this._data = [];
   ...
   * definition.
   */
  _getRowDefs(data: T, dataIndex: number): XcuRowDef<T>[] {
    if (this._rowDefs.length == 1) {
      return [this._rowDefs[0]];
    }

    let rowDefs: XcuRowDef<T>[] = [];
  ...
  private _renderCellTemplateForItem(
    ...
    for (let cellTemplate of this._getCellTemplates(rowDef)) {
      if (XcuCellOutlet.mostRecentCellOutlet) {
        XcuCellOutlet.mostRecentCellOutlet._viewContainer.createEmbeddedView(
          cellTemplate,
          context
        );
      }
  ...
  private _checkStickyStates() {
    const stickyCheckReducer = (
      acc: boolean,
      d: XcuHeaderRowDef | XcuFooterRowDef | XcuColumnDef
    ) => {
  ...
  private _updateNoDataRow() {
    ...
    if (noDataRow) {
      ...
      if (shouldShow !== this._isShowingNoDataRow) {
        ...
        shouldShow
          ? container.createEmbeddedView(noDataRow.templateRef)
          : container.clear();
